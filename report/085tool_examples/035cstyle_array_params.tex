...
% We need to convert C-style to std in function calls because different prototype.
% We have an unhandled problem: std array does not decay to pointers, meaning we cant always pass std arrays to previous functions.
There are multiple implications when the entire codebase is refactored to use \cppinline{std::array}s instead of CStyle arrays. These implications occur because CStyle arrays and \cppinline{std::array}s have small differences in their semantics.
For one CStyle arrays implicitly decay to pointers when used as function parameters, while \cppinline{std::array}s are strong types that do not decay. In order to investigate the semantic implications and challenges which arise when doing the conversion, a modification was made to the tool, which makes the tool also convert function parameters into \cppinline{std::array}s.
