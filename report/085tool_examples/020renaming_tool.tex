...
As can be seen in \cref{code:080dev:tool_example:tool_invocation} the tool combines the results from the other parts of the tool structure into the final tool. The tool is then invoced by the \cppinline{runAndSave} method, which runs the tool and saves the results to disk afterwards. 
