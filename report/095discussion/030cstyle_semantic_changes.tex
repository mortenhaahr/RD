...
For instance, one tool can check if all the calls to a given function, which takes a C-style array parameter, are made with \cppinline{data()} methods on \cppinline{std::array}s with the same size. In this scenario, the function parameter could be directly transformed, similar to the current version of the tool. 
